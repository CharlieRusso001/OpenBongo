...
bool BonkPackConfig::loadFromFile(const std::string& configPath, BonkPackConfig& config) {
    ...
    while (std::getline(file, line)) {
        ...
        else if (keyLower == "bonksound" || keyLower == "bonk_sound") {
            config.bonkSound = value;
        }
    ...
    file.close();
    return !config.name.empty() && !config.bonkSound.empty();
}

std::string BonkPackConfig::getSoundPath(const std::string& soundName) const {
...
std::string BonkPackConfig::getImagePath(const std::string& imageName) const {
